...
function extractFacilities(source) {
  const facilities = [];
  const push = (facility) => { if (facility) facilities.push(facility); };
  const anchorRe = /<a\b[^>]*href=["']([^"']*facility_view\?[^"']*)["'][^>]*>([\s\S]*?)<\/a>/gi;
  for (const match of source.matchAll(anchorRe)) push(facilityFromUrl(match[1], match[2]));
  const hrefRe = /(?:href|data-url)=["']([^"']*facility_view\?[^"']*)["']/gi;
  for (const match of source.matchAll(hrefRe)) push(facilityFromUrl(match[1]));
  const bareRe = /sports_facilities\/facility_view\?[^"'<>\\\s)]+/gi;
  for (const match of source.matchAll(bareRe)) push(facilityFromUrl(match[0]));
  return facilities;
}

function extractFacilityListUrls(source) {
  const urls = [];
...
async function loadFacilityCatalog() {
  ...
  while (queue.length && visited.size < 25) {
    ...
    try {
      ...
      discovered.push(...extractFacilities(source));
      for (const nextUrl of extractFacilityListUrls(source)) {
